 ...
 * Date: 2020 Oct
 *
 * This program runs a grid of stellar models
 *
 */
...
#include "runPcPhicGrid-cmdline.h"
...
int main(int argc, char *argv[])
{
  ...
  //////////////////////////////
  // phic grid
  const int n_phi = args_info.n_phi_arg;
  if (n_phi < 1)
  {
    std::cerr << "ERROR: n-phi *must* be at least one!" << std::endl;
    return 1;
  };
  ...
  const double delta_phic = (n_phi > 1) ? (phic_high-phic_low)/(n_phi-1.) : 0.;
  ...
  // Model variables
  double pc =  pc_low;
  double phic =  phic_low;
  BackgroundModel model0( EoS, conf, pc, phic );

  unsigned int i=0;
  const unsigned int tot = n_p*n_phi;

  for ( unsigned int ip=0; ip < n_p; ip++ ) {
    pc = pow( 10., log10_pc_low + ip*delta_pc );

    for ( unsigned int iphi=0; iphi < n_phi; iphi++, i++ ) {
      phic = phic_low + iphi*delta_phic;
